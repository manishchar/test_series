...
class Zeroreport extends CI_Controller {
    ...
    {
        $data['datalist']=$this->admin->getRows('SELECT s.*, SUM(fp.amount) as Total FROM student s,fees_payment fp WHERE s.id = fp.s_id  and s.status =1 GROUP BY s.id ');
        $data['template']='admin/zeroreport/index';
