...
class Datereport extends CI_Controller {
    ...
    public function index()
    {
         $id=$this->input->post('id');
          $id=$this->input->post('id');
           $id=$this->input->post('id');
            $id=$this->input->post('id');
        $data['datalist']=$this->admin->getRows('SELECT  s.*,b.startdate,b.starttime FROM student s,batch b WHERE s.status = 1 and s.batch_id = b.id  GROUP BY s.batch_id');
