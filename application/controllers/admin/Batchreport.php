...
class Batchreport extends CI_Controller {
    ...
    {
        $data['datalist']=$this->admin->getRows('SELECT s.*,group_concat(s.id) as sid,b.startdate,b.starttime,b.endtime,a.name as f_name FROM student s,batch b  JOIN admin as a on b.faculty_id = a.id  WHERE s.IsDeleted = 0 and s.status = 1 and s.batch_id = b.id  GROUP BY s.batch_id');
        // echo "<pre>";
  ...
  public function detail($batch)
    {
         $data['datalist']=$this->admin->getRows('SELECT s.* FROM student s,batch b WHERE s.IsDeleted = 0 and s.status = 1 and s.batch_id = b.id  and s.batch_id ='.$batch.'');
